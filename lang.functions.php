...
if (!function_exists('get_module_language_file')) {include(WB_PATH.'/framework/module.functions.php');}


    function getLangFromBrowser ($default_language, $lang_variable = null, $strict_mode = true) {
      $allowed_langs = allowedLanguages();
...
function allowedLanguages() {
    $allowedLangs = glob(WB_PATH.'/languages/??.php', GLOB_BRACE);
    array_walk(
        $allowedLangs,
        function (&$sFile) {
...
//        $return_value = $GLOBALS['wb']->page_link($page['link']);//$link
